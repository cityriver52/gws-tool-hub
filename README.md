...
| スレッドID | 親投稿ID | 初回投稿日 | 最終更新日時 | スレッド本文 | タイトル | 紹介文 | タグ | 掲載判定 | AI処理状態 | AI処理日時 | AI処理用本文 |
|---|---|---|---|---|---|---|---|---|---|---|---|

`スレッド本文` はGoogle Chatから取得した原文を保持します。`AI処理用本文` はGASがURLを `[リンク省略]` に置き換えたテキストで、Workspace Studioはこの列だけをAI入力として使用します。これにより、投稿中のDrive動画や外部リンクをGeminiが意図せず参照しようとするのを避けます。
...
7. Workspace Studioで `AI処理状態 = 未処理 / 要再処理` のCatalog行を処理する。AI入力には `スレッド本文` ではなく `AI処理用本文` を指定する。
8. Webアプリとしてデプロイする。
...
- 既存行で `AI処理用本文` が空の場合も、次回の `runScheduledSync()`、`runFullSync()`、または `rebuildCatalog()` で補完されます。
- `ChatLink.gs` はGoogle Chatのコピーリンク形式に合わせ、`https://chat.google.com/room/{spaceId}/{threadId}/{messageId}?cls=10` を生成します。
- Workspace Studioの紹介文生成では、URLやハイパーリンク文字列を出力しないようプロンプトでも制約しています。既存データに長いURLが残っていても、Web UI側では折り返してカードからはみ出さないようにしています。
